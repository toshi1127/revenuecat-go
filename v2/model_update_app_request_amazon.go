/*
Developer API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v2

import (
	"encoding/json"
)

// checks if the UpdateAppRequestAmazon type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UpdateAppRequestAmazon{}

// UpdateAppRequestAmazon Amazon type details. Should only be used when type is amazon.
type UpdateAppRequestAmazon struct {
	// The package name of the app
	PackageName *string `json:"package_name,omitempty"`
	// Your Amazon Developer Identity Shared Key
	SharedSecret NullableString `json:"shared_secret,omitempty"`
}

// NewUpdateAppRequestAmazon instantiates a new UpdateAppRequestAmazon object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUpdateAppRequestAmazon() *UpdateAppRequestAmazon {
	this := UpdateAppRequestAmazon{}
	return &this
}

// NewUpdateAppRequestAmazonWithDefaults instantiates a new UpdateAppRequestAmazon object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUpdateAppRequestAmazonWithDefaults() *UpdateAppRequestAmazon {
	this := UpdateAppRequestAmazon{}
	return &this
}

// GetPackageName returns the PackageName field value if set, zero value otherwise.
func (o *UpdateAppRequestAmazon) GetPackageName() string {
	if o == nil || IsNil(o.PackageName) {
		var ret string
		return ret
	}
	return *o.PackageName
}

// GetPackageNameOk returns a tuple with the PackageName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateAppRequestAmazon) GetPackageNameOk() (*string, bool) {
	if o == nil || IsNil(o.PackageName) {
		return nil, false
	}
	return o.PackageName, true
}

// HasPackageName returns a boolean if a field has been set.
func (o *UpdateAppRequestAmazon) HasPackageName() bool {
	if o != nil && !IsNil(o.PackageName) {
		return true
	}

	return false
}

// SetPackageName gets a reference to the given string and assigns it to the PackageName field.
func (o *UpdateAppRequestAmazon) SetPackageName(v string) {
	o.PackageName = &v
}

// GetSharedSecret returns the SharedSecret field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *UpdateAppRequestAmazon) GetSharedSecret() string {
	if o == nil || IsNil(o.SharedSecret.Get()) {
		var ret string
		return ret
	}
	return *o.SharedSecret.Get()
}

// GetSharedSecretOk returns a tuple with the SharedSecret field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *UpdateAppRequestAmazon) GetSharedSecretOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SharedSecret.Get(), o.SharedSecret.IsSet()
}

// HasSharedSecret returns a boolean if a field has been set.
func (o *UpdateAppRequestAmazon) HasSharedSecret() bool {
	if o != nil && o.SharedSecret.IsSet() {
		return true
	}

	return false
}

// SetSharedSecret gets a reference to the given NullableString and assigns it to the SharedSecret field.
func (o *UpdateAppRequestAmazon) SetSharedSecret(v string) {
	o.SharedSecret.Set(&v)
}
// SetSharedSecretNil sets the value for SharedSecret to be an explicit nil
func (o *UpdateAppRequestAmazon) SetSharedSecretNil() {
	o.SharedSecret.Set(nil)
}

// UnsetSharedSecret ensures that no value is present for SharedSecret, not even an explicit nil
func (o *UpdateAppRequestAmazon) UnsetSharedSecret() {
	o.SharedSecret.Unset()
}

func (o UpdateAppRequestAmazon) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UpdateAppRequestAmazon) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.PackageName) {
		toSerialize["package_name"] = o.PackageName
	}
	if o.SharedSecret.IsSet() {
		toSerialize["shared_secret"] = o.SharedSecret.Get()
	}
	return toSerialize, nil
}

type NullableUpdateAppRequestAmazon struct {
	value *UpdateAppRequestAmazon
	isSet bool
}

func (v NullableUpdateAppRequestAmazon) Get() *UpdateAppRequestAmazon {
	return v.value
}

func (v *NullableUpdateAppRequestAmazon) Set(val *UpdateAppRequestAmazon) {
	v.value = val
	v.isSet = true
}

func (v NullableUpdateAppRequestAmazon) IsSet() bool {
	return v.isSet
}

func (v *NullableUpdateAppRequestAmazon) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpdateAppRequestAmazon(val *UpdateAppRequestAmazon) *NullableUpdateAppRequestAmazon {
	return &NullableUpdateAppRequestAmazon{value: val, isSet: true}
}

func (v NullableUpdateAppRequestAmazon) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpdateAppRequestAmazon) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


